...
async function getCachedPayload(cache) {
  if (cache.getPayload) {
    return cache.getPayload();
  }
  return cache.get(CACHE_KEYS.payload, 'json');
}

async function putRefreshStatus(cache, status) {
  if (cache.putStatus) {
    await cache.putStatus(status);
    return;
  }
  await cache.put(CACHE_KEYS.status, JSON.stringify(status, null, 2));
}

async function putRenderedSite(cache, { html, payload, status }) {
  if (cache.putRenderedSite) {
    await cache.putRenderedSite({ html, payload, status });
    return;
  }
  await Promise.all([
    cache.put(CACHE_KEYS.html, html),
    cache.put(CACHE_KEYS.payload, JSON.stringify(payload, null, 2)),
    cache.put(CACHE_KEYS.status, JSON.stringify(status, null, 2)),
  ]);
}

async function refreshSite(cache, peopleMap, options = {}) {
